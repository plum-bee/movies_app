...
class ApiService {
  ...
  static Future<Actor?> getActorDetails(int actorId) async {
    try {
      http.Response detailsResponse = await http.get(Uri.parse(
          '${Api.baseUrl}${ApiEndPoints.actorDetails}$actorId?api_key=${Api.apiKey}&language=en-US'));
      var detailsData = jsonDecode(detailsResponse.body);

      http.Response filmographyResponse = await http.get(Uri.parse(
          '${Api.baseUrl}${ApiEndPoints.actorFilmography.replaceFirst('{actor_id}', actorId.toString())}?api_key=${Api.apiKey}&language=en-US'));
  ...
  static Future<List<Actor>?> getTrendingActors(String period) async {
    List<Actor> actors = [];
    try {
      http.Response response = await http.get(Uri.parse(
          '${Api.baseUrl}${ApiEndPoints.trendingActors}/$period?api_key=${Api.apiKey}&language=en-US'));
      var res = jsonDecode(response.body);
      if (res['results'] != null) {
        res['results'].forEach(
          (a) => actors.add(
